...
class SRegessionNet(nn.Module):
    def __init__(self, backbone, freeze=False):
        ...
        self.conv_1x1 = nn.Sequential(
            nn.Conv3d(128+256+512+512, 512, kernel_size=1),
            nn.BatchNorm3d(512),
            nn.ReLU(True))
    ...
    def forward(self, x):
        ...
        seg = self.seg2(x)
        x = x * seg[:, 0].unsqueeze(1)
        x1 = max_pool3d(torch.cat([x, x], dim=1), 2)       # (N,128,32,28,28)
        seg_list.append(seg)
        ...
        seg = self.seg3(x)
        x = x * seg[:, 0].unsqueeze(1)
        x2 = max_pool3d(torch.cat([x, x], dim=1), 2)       # (N,256,16,14,14)
        seg_list.append(seg)

        x = self.backbone.layer3(x)     # (N,256,16,14,14)
        x = x + x2
        seg = self.seg4(x)
        x = x * seg[:, 0].unsqueeze(1)  # (N,512,16,14,14)
        x3 = max_pool3d(torch.cat([x, x], dim=1), 2)       # (N,512,8,7,7)
        seg_list.append(seg)
        ...
        x = torch.cat([x1, x2, x3, x], dim=1)       # (N,128+256+512+512,8,7,7)
