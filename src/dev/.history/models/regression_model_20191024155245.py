...
class SRegessionNet(nn.Module):
    ...
    def forward(self, x):
        ...
        img = (img-img.min())/(img.max()-img.min())
        viz.image(cmap(img.detach().cpu().numpy())[..., :3].transpose(
            2, 0, 1), opts=dict(height=224, width=224, title=f'x*seg2 epoch-200'))
        ...
        img = (img-img.min())/(img.max()-img.min())
        viz.image(cmap(img.detach().cpu().numpy())[..., :3].transpose(
            2, 0, 1), opts=dict(height=224, width=224, title=f'x*seg3 epoch-200'))
        ...
        img = (img-img.min())/(img.max()-img.min())
        viz.image(cmap(img.detach().cpu().numpy())[..., :3].transpose(
            2, 0, 1), opts=dict(height=224, width=224, title=f'x*seg4 epoch-200'))
        ...
        img = (img-img.min())/(img.max()-img.min())
        viz.image(cmap(img.detach().cpu().numpy())[..., :3].transpose(
            2, 0, 1), opts=dict(height=224, width=224, title=f'x*seg5 epoch-200'))
