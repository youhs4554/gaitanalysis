...
class DefaultAGNet(GeneralizedAGNet):
    def __init__(self,
        ...
        super(DefaultAGNet, self).__init__(guider,
...
class ConcatenatedAGNet(GeneralizedAGNet):
    def __init__(self,
                 backbone, pretrained_agnet, n_outputs=2, task='classification',
                 backbone_dims=[64, 64, 128, 256, 512], target_transform=None):
        ...
        super(ConcatenatedAGNet, self).__init__(guider,
                                                predictor, target_transform=target_transform)
...
def regional_agnet(opt, backbone, backbone_dims, n_outputs, load_pretrained_agnet=False, target_transform=None):
    net = RegionalAGNet(backbone, n_outputs=n_outputs, task=opt.task,
                        backbone_dims=backbone_dims, target_transform=target_transform)
...
def concatenated_agnet(opt, backbone, backbone_dims, pretrained_agnet, n_outputs, target_transform=None):
    net = ConcatenatedAGNet(backbone, pretrained_agnet,
                            n_outputs=n_outputs, task=opt.task,
                            backbone_dims=backbone_dims, target_transform=target_transform)
    return net
