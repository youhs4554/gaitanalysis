...
import random
from utils import reproductivity_decorator
import warnings
import numpy as np
...
from references.utils_cv.action_recognition.dataset import get_transforms
...
class VideoLearner(object):
    ...
    @staticmethod
    @reproductivity_decorator(seed=0)
    def init_model(
    ...
    def fit(
        ...
        lr: float,
        epochs: int,
        logdir: str = "activity_learner_logs",
        model_name: str = None,
        momentum: float = 0.95,
        ...
        if tsn:
            model_name = model_name + "_tsn"

        from datetime import datetime
        version_dir = os.path.join(logdir,
                                   model_name, datetime.strftime(datetime.now(), "%Y%m%d_%H%M%S"))
        model_dir = os.path.join(version_dir, "checkpoints")

        from torch.utils.tensorboard import SummaryWriter
        tb_writer = SummaryWriter(version_dir)
        ...
        for e in range(1, self.epochs + 1):
            ...
            tb_writer.add_scalar("lr", scheduler.get_lr()[0], e)

            current_score = self.results[-1][f"valid/top1"]
            if current_score > best_score:
                # update best model
                best_model = self.model
                print(
                    f"Update best model! (top1 : {best_score:.4f}->{current_score:.4f})")
    ...
    def train_an_epoch(
        ...
        for phase in ["train", "valid"]:
            ...
            for step, (inputs, target) in enumerate(dl, start=1):
                ...
                if tsn:
                    inputs = inputs.view(-1, 3, *inputs.size()[-3:])

                with torch.set_grad_enabled(phase == "train"):
                    if getattr(model, "distilling", None):
                        outputs, pred_from_student, target_from_teacher = model(
                            inputs)
                        if tsn:
                            # consensus
                            outputs = outputs.view(
                                batch_size, -1, outputs.size(-1)).mean(1)
                        loss = criterion(
                            outputs, target, pred_from_student, target_from_teacher
                        )
                    else:
            ...
            tb_writer.add_scalar(f"{phase}/loss", losses.avg, epoch)
            tb_writer.add_scalar(f"{phase}/top1", top1.avg.item(), epoch)
            tb_writer.add_scalar(f"{phase}/top5", top5.avg.item(), epoch)
    ...
    def evaluate(
        ...
        guide_method = getattr(self, "guide_method", None)
        if guide_method == "distillation":
            # use student for evaluation
