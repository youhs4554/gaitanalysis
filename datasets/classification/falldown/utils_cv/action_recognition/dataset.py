...
def get_transforms(train: bool = True, tfms_config: Config = None) -> Trans:
    ...
    tfms = [
        ...
        transforms.ResizeVideo(tfms_config.im_scale,
...
class VideoDataset:
    ...
    def __init__(
        ...
        if train_split_file:
            ...
            assert test_split_file is not None and Path(
                test_split_file).exists()

        if test_split_file:
            assert Path(test_split_file).exists()
            assert train_split_file is not None and Path(
                train_split_file).exists()
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.tensor, int]:
        ...
        if self.num_samples == 1:
            ...
            return (
                # [S, T, H, W, C] -> [S, C, T, H, W]
                torch.stack([self.transforms(torch.from_numpy(c))
                             for c in clips]),
                record.label,
            )
    ...
    def _show_batch(
        ...
        fig, axs = plt.subplots(
            batch_size, sample_length, figsize=(
                4 * sample_length, 3 * batch_size),
        )
        ...
        for i, ax in enumerate(axs):
            ...
            for j, a in enumerate(ax):
                a.axis("off")
                a.imshow(np.moveaxis(denormalize(
